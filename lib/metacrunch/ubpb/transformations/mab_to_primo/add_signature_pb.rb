...
class Metacrunch::UBPB::Transformations::MabToPrimo::AddSignaturePb < Metacrunch::Transformator::Transformation::Step
  def call
    target ? Metacrunch::Hash.add(target, "signature_pb", signature_pb) : signature_pb
  end
  ...
  def signature_pb
    ...
    #
    signatures << source.datafields('200', ind1: ' ', ind2: '9')
    ...
    # ansonsten extrahiere aus den normalen Signaturen eine Basis-Signatur
      ...
      fields.each do |field|
        field.subfields(["b", "d", "f"]).each do |_subfield|
